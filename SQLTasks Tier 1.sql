...
SELECT DISTINCT f.name,
				CONCAT(m.firstname, ' ', m.surname) AS full_booking
FROM Members AS m 
...
SELECT 
	...
	f.name
CASE WHEN b.memid = 0 
AND (b.slots * f.guestcost > 30) THEN (b.slots * f.guestcost)
...
SELECT 
    DISTINCT (CONCAT(subquery.firstname, ' ', subquery.surname)) AS fullname,
    subquery.name,
    subquery.cost
FROM (
    SELECT 
